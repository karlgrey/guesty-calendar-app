...
// The code capture is greedy {8,} (real codes have always been "HM" + 8, but
// longer codes must not truncate — review finding #2) with a lookahead
// boundary on what ACTUALLY follows the code in live Airbnb HTML with its
// zero-whitespace element joints: the next guest's name or "Gesamtbetrag"
// (an Uppercase-lowercase word start), whitespace, or end-of-string.
// Backtracking off the greedy match is what separates ")HME9WZFQTYEgbert"
// into code "HME9WZFQTY" + name "Egbert" — a fixed lookahead on the next
// amount ("-?[\d.,]+ € EUR") does NOT work, because between code and next
// amount the real mail always carries the guest name (prod regression,
