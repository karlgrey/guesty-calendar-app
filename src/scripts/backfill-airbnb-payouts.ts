...
import { syncAirbnbIcal } from '../jobs/airbnb-mail/sync-ical.js';
...
async function main(): Promise<void> {
  ...
  for (const property of getAllProperties()) {
    ...
    // Refresh availability.block_ref from the iCal feed FIRST — reconcile
    // reads whatever is already in the availability table, which may be
    // stale/pre-fix (see ical-parser.ts's DESCRIPTION-based HM-code
    // extraction). Without this, reconcile silently reconciles against
    // outdated block_ref values instead of the current calendar state.
    const icalResult = await syncAirbnbIcal(property);
    if (!icalResult.success) {
      console.log(`  ✗ ${property.slug}: iCal sync failed (${icalResult.error}) — skipping reconcile for this property`);
      continue;
    }
