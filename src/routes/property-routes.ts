...
router.get('/:slug', resolveProperty, (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const propertyScript = `<script>window.__PROPERTY_SLUG__ = "${property.slug}"; window.__PROPERTY_NAME__ = "${property.name}"; window.__BOOKING_EMAIL__ = "${property.bookingRecipientEmail}"; window.__PROPERTY_WEBSITE__ = "${property.website || ''}"; window.__CHECKIN_TIME__ = "${property.checkInTime || ''}"; window.__CHECKOUT_TIME__ = "${property.checkOutTime || ''}";</script>`;
