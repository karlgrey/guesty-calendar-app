...
- **Hostex Owner-Blocks (seit #725, 09/2026):** `GET /api/agent/availability/:slug?from=&to=` — Kalendertage + zu Bereichen gruppierte Blocks (`blockedRanges`) für ein `provider=hostex`-Objekt (Bootshaus, Alte Schilderwerkstatt); ohne `from`/`to` Default heute (Europe/Berlin) bis +365 Tage. `POST /api/agent/availability/:slug/block` bzw. `.../unblock` (Body `{"from","to"}`, beide `YYYY-MM-DD` inklusive) sperren/geben den Hostex-Kalender frei. Alle drei 400 bei `provider≠hostex` (Guesty-Blocks sind ein separater, noch nicht gebauter Zweig), 404 bei unbekanntem Slug, 400 bei ungültigem/rückwärtigem/zu großem (>400 Tage) oder vergangenem Zeitraum. **Hostex verarbeitet die Mutation asynchron** (kein Block-Objekt/keine ID in der Antwort, Channel-Sync nicht garantiert) — Stand danach per GET prüfen.
